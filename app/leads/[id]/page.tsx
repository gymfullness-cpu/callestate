...
type LeadStatus = "Nowy" | "Oddzwoń!" | "Zamknięty";

function normalizeLeadStatus(s: unknown): LeadStatus {
  return s === "Nowy" || s === "Oddzwoń!" || s === "Zamknięty" ? s : "Nowy";
}
...
export default function LeadPage() {
  ...
  return (
    <main style={{ padding: 40 }}>
      ...
      <p>
        ...
        <select
          ...
          onChange={(e) =>
            saveLead({
              ...lead,
              role: (e.target.value as LeadRole) || undefined,
            })
          }
        >
      ...
      <p>
        <strong>Telefon:</strong> <a href={`tel:${lead.phone}`}>{lead.phone}</a>
      </p>
        ...
        return (
          <ul>
            {meetings.map((m: any) => (
              <li key={m.id}>
                {m.date} {m.time}{" "}
                {m.type === "pozyskowe" ? "📥 Pozyskowe" : "🏠 Prezentacja"}
              </li>
            ))}
          </ul>
        );
